...
from structs import Buffer, Sample, History
...
class Sampler:
    ...
    def retrieve_latest_samples(self, buffer, history):
        def retrieve_sample_fori_body(ii, carry):
            buffer, sample = carry
            ...
            sample = sample.at[ii].set(buffer[shifted_ii])

            return buffer, sample

        shift = buffer.head

        retrieve_size = history.obs.shape[1]

        update_func = lambda b, s: jax.lax.fori_loop(0, retrieve_size, retrieve_sample_fori_body, (b, s))[1]
        vmap_update_func = jax.vmap(update_func, in_axes = (0, 0))
